...
{
    [TestClass]
    public class SocketC2Tests
    {
        private readonly string _socketIp = "127.0.0.1"; // Set to Cobalt Strike External C2 server IP
        private readonly string _socketPort = "2222";
        private SocketC2 _beacon = new SocketC2();
        ...
        public void BeaconCanConfigure()
        {
            ...
            {
                // Configure beacon
                _beacon.Configure(_socketIp, _socketPort);

                // Check that the channels were instaniated
                Assert.IsInstanceOfType(_beacon.BeaconChannel, typeof(BeaconChannel));
                Assert.IsInstanceOfType(_beacon.ServerChannel, typeof(SocketChannel));

                // Check that the property values are correct
                Assert.IsNotNull(_beacon.IpAddress);
                Assert.AreEqual(_socketIp, _beacon.IpAddress);
                Assert.IsNotNull(_beacon.Port);
                Assert.AreEqual(_socketPort, _beacon.Port);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Failed: {ex.Message}");
